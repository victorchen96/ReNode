...
def run_batch(model, xbs, yb, optimizer, train,rn_weight=[]):
    ...
    with torch.set_grad_enabled(train):
        ...
        loss = F.cross_entropy(pred, yb, reduction='none')
        loss = loss * rn_weight
        loss = loss.sum()
...
def train(model, train_set, val_set, lr, weight_decay,
          ...
          eval_step=1, early_stop=False, patience=50, ex=None,rn_weight=[]):
    ...
    #batch size is large enough to contain all samples in one batch
    ...
    for epoch in range(max_epochs):
        for xbs, yb in train_loader:
            ...
            loss_batch, ncorr_batch = run_batch(model, xbs, yb, optimizer, train=True,rn_weight=rn_weight)
            loss += loss_batch
            ...
            if step % eval_step == 0:
                ...
                train_acc = ncorrect.item() / nsamples
